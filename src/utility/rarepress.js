...
(() => {
  ...
  class Token extends Thing {
    ...
    async create(body) {
      ...
      if (!body.metadata.name || body.metadata.name.length === 0)
        body.metadata.name = '';
      if (!body.metadata.description || body.metadata.description.length === 0)
        body.metadata.description = '';
      if (!body.metadata.image || body.metadata.image.length === 0)
        body.metadata.image = '';
      if (body.tokenId) {
